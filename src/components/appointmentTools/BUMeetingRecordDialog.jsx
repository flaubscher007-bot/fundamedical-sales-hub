...
import {
  Upload, Loader2, Wand2, FileAudio, Sparkles, Mic, MicOff,
  MapPin, Paperclip, X, CheckSquare, Square, FileText
} from "lucide-react";
...
export default function BUMeetingRecordDialog({ open, onClose, appointment, existing, user }) {
  ...
  const toggleService = (key) => {
    setForm(f => ({
      ...f,
      bu_services: { ...f.bu_services, [key]: !f.bu_services[key] }
    }));
  };
  ...
  const startRecording = async () => {
    ...
    mr.onstop = async () => {
      ...
      setUploading(false);
      // Capture geotag when recording starts if not already set
    };
    mr.start();
    setMediaRecorder(mr);
    setRecording(true);
    // Auto-geotag on record
  ...
  const handleAttachment = async (e) => {
    ...
    const urls = await Promise.all(
      files.map(file => base44.integrations.Core.UploadFile({ file }).then(r => ({ url: r.file_url, name: file.name })))
    );
    setForm(f => ({
      ...f,
      attachment_urls: [...(f.attachment_urls || []), ...urls.map(u => u.url)]
    }));
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab}>
          ...
          <TabsContent value="prep" className="space-y-5 pt-4">
            ...
            <div className="flex items-center gap-3 flex-wrap">
              <Button
                ...
                className="flex items-center gap-2 border-[#92F21D] text-[#92F21D]"
                disabled={geoLoading}
              >
              ...
              {!form.city && (
                <Button type="button" size="sm" variant="ghost" onClick={handleGeoTag} disabled={geoLoading}
          ...
          <TabsContent value="record" className="space-y-4 pt-4">
            ...
            <div className="border border-[#34CCD0]/30 rounded-xl p-5">
              ...
              <div className="flex items-center gap-4">
                {!recording ? (
                  ...
                  <Button
                    ...
                    variant="outline"
                    className="border-red-500 text-red-500 flex items-center gap-2 animate-pulse"
                  >
                    <MicOff className="w-4 h-4" /> Stop Recording
                  </Button>
                )}
                {uploading && (
                  <span className="text-sm flex items-center gap-1" style={{ color: "#34CCD0" }}>
              ...
              <p className="text-xs mt-2 text-slate-400">Recording will auto-geotag your location when started.</p>
            </div>
            ...
            <div className="border border-[#34CCD0]/30 rounded-xl p-5">
              ...
              {form.recording_url ? (
                <div className="flex items-center gap-3">
                  ...
                  <a href={form.recording_url} target="_blank" rel="noreferrer"
                    className="text-sm underline" style={{ color: "#34CCD0" }}>View recording file</a>
                </div>
              ) : null}
              <input ref={audioRef} type="file" accept="audio/*,video/*" className="hidden" onChange={handleAudioUpload} />
              <Button variant="outline" size="sm" className="mt-2" onClick={() => audioRef.current.click()} disabled={uploading}>
              ...
              <div className="flex items-center gap-3">
                <Button onClick={transcribe} disabled={transcribing} className="bg-purple-600 hover:bg-purple-700">
                  {transcribing
                    ...
                    : <><Wand2 className="w-4 h-4 mr-2" /> AI Transcribe</>
                  }
                </Button>
              </div>
            )}
          ...
          <TabsContent value="notes" className="space-y-4 pt-4">
            ...
            <div>
              ...
              <input ref={attachRef} type="file" multiple accept="image/*,.pdf,.doc,.docx,.xlsx,.xls,.ppt,.pptx"
                className="hidden" onChange={handleAttachment} />
                  ...
                  {form.attachment_urls.map((url, idx) => (
                    <div key={idx} className="flex items-center gap-2 p-2 rounded-lg border border-[#34CCD0]/30">
                      ...
                      <a href={url} target="_blank" rel="noreferrer"
                        className="text-xs flex-1 truncate underline" style={{ color: "#34CCD0" }}>
                        Attachment {idx + 1}
                      </a>
